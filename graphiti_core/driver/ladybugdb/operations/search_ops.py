...
from graphiti_core.driver.ladybugdb.operations.record_parsers import (
...
class LadybugDBSearchOperations(SearchOperations):
    # --- Node search ---
    ...
    async def node_fulltext_search(
        ...
        filter_queries, filter_params = node_search_filter_query_constructor(
            search_filter, GraphProvider.LADYBUGDB
        )
        ...
        cypher = (
            get_nodes_query(
                'node_name_and_summary', '$query', limit=limit, provider=GraphProvider.LADYBUGDB
            )
            ...
            RETURN
            """
            + get_entity_node_return_query(GraphProvider.LADYBUGDB)
        )
    ...
    async def node_similarity_search(
        ...
        cypher = (
            ...
            WITH n, """
            + get_vector_cosine_func_query(
                'n.name_embedding', search_vector_var, GraphProvider.LADYBUGDB
            )
            + """ AS score
    ...
    async def edge_fulltext_search(
        ...
        cypher = (
            get_relationships_query(
                'edge_name_and_fact', limit=limit, provider=GraphProvider.LADYBUGDB
            )
            + """
            ...
            RETURN
            """
            + get_entity_edge_return_query(GraphProvider.LADYBUGDB)
            + """
            ORDER BY score DESC
            LIMIT $limit
            """
    ...
    async def edge_similarity_search(
        ...
        cypher = (
            ...
            WITH DISTINCT e, n, m, """
            + get_vector_cosine_func_query(
                'e.fact_embedding', search_vector_var, GraphProvider.LADYBUGDB
            )
            + """ AS score
            WHERE score > $min_score
            RETURN
    ...
    async def episode_fulltext_search(
        ...
        cypher = (
            get_nodes_query(
                'episode_content', '$query', limit=limit, provider=GraphProvider.LADYBUGDB
            )
            + """
    ...
    async def community_fulltext_search(
        ...
        cypher = (
            get_nodes_query(
                'community_name', '$query', limit=limit, provider=GraphProvider.LADYBUGDB
            )
            + """
    ...
    async def community_similarity_search(
        ...
        cypher = (
            ...
            """
            + get_vector_cosine_func_query(
                'c.name_embedding', search_vector_var, GraphProvider.LADYBUGDB
            )
            + """ AS score
            WHERE score > $min_score
            RETURN
    ...
    def build_node_search_filters(self, search_filters: SearchFilters) -> Any:
        filter_queries, filter_params = node_search_filter_query_constructor(
            search_filters, GraphProvider.LADYBUGDB
        )
